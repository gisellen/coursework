...
PURPOSE
The purpose of this lab is to show how linked lists are created in a basic way and used.  There are a set of options that will be implemented in this code
to manipulate the linked list based on the option that is chosen.  The options that can be chose is:
o  searching the list if a number exists
o  adding a number to the beginning of the list
o  deleting a number for the list

The input will consist of numbers that will be inputted into the linked list.

The output will show a program that the user can choose from until the user chooses to exit the program.
*/
...
void deletenode(Node* list2, Node* headptr2, int num2) { //this function will delete a node from the list
    ...
    while (search1 == false || missing == false) {
        ...
        //if the number does not exist, then output that it doesnt exist
        if (del->next == nullptr) {
            cout << "There is no such item in the list." << endl;
            missing = true;
        }

        //continue searching through the list if it is not equal
